...
\centering
	\begin{tabular}{l|c|c|c}
		...
		Verwendungszweck & 27\% & & \\
		\hline
		\textbf{Gesamtwertungszahl} & \textbf{100\%} & & \\
	\end{tabular}
\caption{Entscheidungsmatrix der Frameworkart}
\label{tab:EntscheidungsmatrixFrameworkart}
\end{table} \\
Die \autoref{tab:EntscheidungsmatrixFrameworkart} bildet eine Hilfestellung zur Entscheidung einer passenden Frameworkart für das Projekt. 


\subsection{Flask}
\label{subsec:Flask}
Wie unter \autoref{subsec:GuiPython} beschrieben, wird im Rahmen der Studienarbeit Flask verwendet. Dabei ist Flask keine Software zur Erstellung einer Grafischen Oberfläche. Es ist ein Python Web-Framework welches eine Schnittstelle zum Webserver bildet. Bekannt ist Flask durch die Eigenschaft schnell und leicht eine minimale Anwendung zu implementieren und diese bei bedarf in der Größe zu skalieren.

In welcher Form Flask verwendet werden kann, ist im folgenden Listing dargestellt.
\begin{lstlisting}[language=Python,caption=Example Listing of Flask Python,label={lst:Flask}]
...
def homepage():
	return 'Go Chat!'
\end{lstlisting}
Das Listing spiegelt die Moduleigenschaft von Python aufgrund des Paketverwaltungsprogramm pip wieder. Dementsprechend wird Flask in der ersten Zeile importiert, einer Anwendung zugeordnet und eine Route festgelegt.
