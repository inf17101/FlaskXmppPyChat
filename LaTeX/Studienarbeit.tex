...
Die Aufgabe des Reverse Proxys ist es Anfragen von Servern stellvertretend anzunehmen und an den entsprechenden Client weiterzuleiten \cite{reverseProxyDesc}. Der Proxy nimmt Anfragen von Clients an das \ac{IMS} entgegen und leitet diese Anfrage an die Flask \acs{REST} \acs{API} weiter.


Noch nicht fertig ab hier...

Abbildung \autoref{img:ArchitekturIMS} im Anhang zeigt den physikalische Netzwerktopologie des \ac{IMS}. Der ejabberd-Server ist in \autoref{img:ArchitekturIMS} als \texttt{ejabberd01} gekennzeichnet und der Apache Kafka Dienst als \texttt{apkafka01}. Die Instanz \texttt{ims-webapp01} realisiert NGINX, die Flask Applikation und die MySQL-Datenbank für Benutzerdaten.
